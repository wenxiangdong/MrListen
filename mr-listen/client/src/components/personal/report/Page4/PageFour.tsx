    ...
    let reportInfo = minutes > 0
      ...
      : seconds
        ? (
          <View>
            <View className={`align-text`}>
              <View className={`text-align-last word-1 ${mounted ? `word-fly-in-1` : ``}`}>你倾诉最长的一次</View>
              <View className={`text-align-last word-2 ${mounted ? `word-fly-in-3` : ``}`}>
                <Text>也不过说了{'\n'}</Text>
              </View>
              <View className={`word-3 ${mounted ? `word-fly-in-5` : ``}`}>
        ...
        : (
          <View className={`align-text`}>
            <View className={`text-align-last word-5 ${mounted ? `word-fly-in-1` : ``}`}>你还没有给我发过消息</View>
            <View className={`text-align-last strong word-6 ${mounted ? `word-fly-in-4` : ``}`}>我其实很想听你讲故事</View>
            <NextPageTip flyInTime={"6"} bottom={'120rpx'}/>
          </View>
        )
    ;
